...
class KannatusseuraCat(models.Model):
    ...
    _sql_constraints = [
        ('unique_code_unique', 'unique(kannatusseura_cat_code)', 'Category code already in use, try a different one.')
    ]
    ...
    def _check_unique_code(self):
        for record in self:
            if record.kannatusseura_cat_code:
                ...
                if existing:
                    raise ValidationError("This category code is already assigned to another contact.")
